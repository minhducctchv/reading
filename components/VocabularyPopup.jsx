 ...
 * }} props
 */
export function VocabularyPopup({ vocab, selectedText, position, onClose, onSaved, onDeleted }) {
    const [originalWord, setOriginalWord] = useState(vocab?.originalWord ?? '')
    const [text, setText] = useState(vocab?.text ?? selectedText ?? '')
    const [pronunciation, setPronunciation] = useState(vocab?.pronunciation ?? '')
    ...
    async function handleDelete() {
        ...
        setLoading(true)
        try {
            const res = await fetch(`/api/vocabulary?id=${vocab._id}`, { method: 'DELETE' })
            const json = await res.json()
            if (!res.ok) throw new Error(json.error || 'Failed')
            onDeleted(vocab._id)
        } catch (err) {
        ...
        <div className="vocab-popup-backdrop">
            ...
            >
                <div className="vocab-popup-header">
                    ...
                    <button className="vocab-popup-close" onClick={onClose} aria-label="Close">✕</button>
                </div>
                ...
                <form onSubmit={handleSave} className="vocab-popup-form">
                    ...
                    <div className="vocab-field">
                        <label>Meaning <span className="required">*</span></label>
                        <textarea
                            value={meaning}
